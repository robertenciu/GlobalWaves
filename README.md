...
Nextly, it iterates through the input commands and takes the current user. Creates a new inputProcessor and assuming each user has his own individual search bar, player and status, the inputProcessor class holds the reference to them. Depending on the command name, the switcher chooses what action to take (example: search, select, etc.) and calls the specific method from the inputProcessor.
...
* Statistics methods get the instance of the Statistics (since it is SINGLETON) and output the media list.
